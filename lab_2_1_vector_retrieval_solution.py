...
Compare the retrievers using the same test questions: keyword retrieval from Lab 1.2 performs well when queries closely match terms in the source material, while vector retrieval in this lab can perform better when queries use different wording or paraphrases. In Lab 2.2, you will combine both approaches in a hybrid retriever.
"""
import os
import sys
...
class VectorRetriever(BaseRetriever):
    ...
    def retrievedContext(self, query: str) -> str:
        ...
        # The retrieved set is deliberately small, but the model also needs to
        # know the size of the complete corpus for questions about the database.
        corpus_size = self._db._collection.count()
        emails = "\n\n---\n\n".join(
            f"[{name}]\n{content}" for name, content, _ in results
        )
        return (
            f"Database metadata: the complete email corpus contains {corpus_size} emails. "
            f"The {len(results)} emails below are the ones retrieved for this question.\n\n"
            f"{emails}"
        )
